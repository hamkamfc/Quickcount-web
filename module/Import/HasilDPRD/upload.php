...
// insert to log
//$q = " select 1 from imported where type = 2 and filename = ? ";
...
//if ($is_exist > 0) {
//	$q = " update hasil_dprd set hasil = ? where caleg_id = ? and partai_id = ? ";
//	$ps = Jaring::$_db->prepare ($q);
//
//	foreach ($data as $in) {
//		$i = 1;
...
//} else {
	...
	$q	=" insert into hasil_dprd (dapil_id, kecamatan_id, kelurahan_id, tps_id, kode_saksi, caleg_id, partai_id, hasil"
		.") values ("
		." ?, ?, ?, ?, ?, ?, ?, ?"
		.")";
	$ps = Jaring::$_db->prepare ($q);
...
$q = " insert into imported (type, filename) values ( 2 , ? )";
$ps = Jaring::$_db->prepare ($q);
